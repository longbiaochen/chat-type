...
> ChatType is a native macOS dictation app for people already signed in to ChatGPT/Codex Desktop on this Mac. Press F5 to record, press F5 again to stop, then paste into the focused editable target when safe or keep the transcript in the clipboard.

Do not describe the default route as a stable public API. ChatType v1 depends on a local desktop login state and an upstream private transcription path.
